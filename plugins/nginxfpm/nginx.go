...
func loadPluginConfig() (*PluginConfig, error) {
	...
	// set domain
	cfg.Domain = os.Getenv("NGINX_DOMAIN")
	if domain == "" {
		return errors.new("NGINX_DOMAIN is a required parameter if using this plugin")
	}
...
func (p NginxPlugin) domainDoesNotMatch(string d) bool {
	return d != p.Info().Domain
}
...
func (p NginxPlugin) generateNginxConfig() (*NginxConfig, error) {
	...
	for _, c := range containers {
		...
		if p.DomainDoesNotMatch(domain) {
			continue
		}
